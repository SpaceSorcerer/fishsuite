...
| `qki_associated_with_miat`          | **CAPACITY (extensive).** Σ over the nucleus's **nuclear** MIAT spots of `qki_at_miat_footprint × miat_footprint_area_px` — the **total (integrated) QKI intensity inside MIAT-spot footprint pixels** (mean × area). |
...
| `capacity_qki_at_miat`              | **CAPACITY (the sponge test).** `qki_associated_with_miat ÷` the **total nuclear QKI intensity** (`nuclear_total_intensity_rna2` → `nuclear_total_intensity_protein`) — the **fraction of the nucleus's QKI captured by MIAT footprints**. NaN when the denominator ≤ 0. |

The three **CAPACITY** columns are the **extensive / mass-based sponge test** — they
ask *how much* QKI is associated with this nucleus's MIAT, and what fraction of the
...
`mean_qki_at_miat_foci_enrichment`, `mean_qki_associated_with_miat`,
